...
def test_legacy_collector_accepts_multi_topic_direction(tmp_path, monkeypatch):
    collector = FixedSourceCollector(
        _write_registry(tmp_path),
        tmp_path / "multi-topic.sqlite",
    )
    listing = '<a href="/news/297.html">因时机器人投产</a>'
    detail = (
        "<article>2026-07-20 具身智能新基地投产，年产能10万台，已量产交付</article>"
    )
    ...
    evidence = collector.collect("具身智能|半导体")

    assert [item.company for item in evidence] == ["因时机器人"]
    assert evidence[0].direction == "具身智能|半导体"
