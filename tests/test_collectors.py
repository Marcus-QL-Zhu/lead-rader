...
def test_false_people_and_generic_job_results_are_rejected():
    ...
    assert not [route for route in routes if route.kind in {'alumni_or_academic', 'former_colleague'}]
...
def test_replay_removes_generic_job_ads_and_bad_routes(tmp_path):
    ...
    replay.write_text(json.dumps([{
        ...
        'evidence': [
            {
                'event_type': 'funding', 'phase': 'strategy_capital', 'event_date': '2026-06-04',
                'title': '戴盟机器人完成亿元A轮融资', 'snippet': '本轮由联想创投投资，用于模型研发和量产',
                'source_url': 'https://example.com/funding', 'source_name': 'source', 'source_grade': 'C',
            },
            {
                'event_type': 'job_ad', 'phase': 'recruit', 'event_date': '2026-05-01',
                'title': '供应链总监招聘', 'snippet': '某高端制造公司',
                'source_url': 'https://example.com/generic-job', 'source_name': 'source', 'source_grade': 'C',
            },
        ],
        'outreach_routes': [
            {'kind': 'alumni', 'target': '之一—某大学', 'path': 'x', 'evidence_url': 'u', 'grade': 'C', 'note': 'n'},
            {'kind': 'investor', 'target': '联想创投', 'path': 'x', 'evidence_url': 'u', 'grade': 'C', 'note': 'n'},
        ],
    }], ensure_ascii=False), encoding='utf-8')
    evidence, metadata = load_replay(replay, '灵巧手')
    assert len(evidence) == 1
    assert evidence[0].event_type == 'factory_or_capacity'
    assert [route['target'] for route in metadata['routes']['戴盟机器人']] == ['联想创投']
...
def test_josint_adapter_accepts_real_field_variants_and_filters_seniority(tmp_path):
    ...
    connection.executemany(
        ...
        [
            ('agency', '灵巧手制造总监', 'https://example.com/1', '', '全面负责团队搭建', '北京示例机器人公司', '2026-07-20', '2026-07-20'),
            ('agency', '灵巧手高级经理', 'https://example.com/2', '', '负责项目', '北京示例机器人公司', '2026-07-20', '2026-07-20'),
        ],
    )
    connection.commit()
    connection.close()
