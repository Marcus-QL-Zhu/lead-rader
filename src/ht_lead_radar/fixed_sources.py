...
from .collectors import _clean_html, _event_date_from_text, extract_company_candidates, infer_event
...
class FixedSourceCollector:
    ...
    def _relevant(self, source: dict, direction: str, title: str, body: str) -> bool:
        ...
        direction_terms = set(profile.aliases) | set(profile.seed_companies)
        event_terms = tuple(profile.discovery_terms) + (
            '融资', '量产', '交付', '订单', '基地', '工厂', '产线', '产能',
            '投产', '签约', '战略合作', '海外', '数据集', '大模型', '发布',
        )
    ...
    def _evidence(self, source: dict, direction: str, url: str, title: str, body: str) -> list[Evidence]:
        text = f'{title} {body}'
        ...
        if source.get('company'):
            ...
            seeds = profile_for(direction).seed_companies
            ...
            suffixes = ('公司', '集团', '科技', '智能', '机器人', '仿生', '航天', '能源')
            title_candidates = [
                name for name in extract_company_candidates(title)
    ...
    def _record_run(self, source_id: str, status: str, count: int, error: str = '') -> None:
        with sqlite3.connect(self.state_db) as connection:
            connection.execute('''
            ...
            try:
                ...
                for url, listing_title in self._candidate_links(source, listing):
                    ...
                    if not self._relevant(source, direction, listing_title, body):
                        continue
                    for item in self._evidence(source, direction, url, listing_title, body):
