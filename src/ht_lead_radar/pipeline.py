...
def _parse_date(value: str) -> date | None:
    if not value:
        return None
    for pattern in (r'(20\d{2})[-/年](\d{1,2})[-/月](\d{1,2})', r'(20\d{2})[-/](\d{1,2})'):
...
def build_leads(
    ...
    for company, items in grouped.items():
        ...
        for item in upstream:
            event_key = item.event_id or f'{item.event_type}|{item.event_date}|{item.source_url}'
            if event_key in seen_events:
                continue
            seen_events.add(event_key)
            weights_by_type[item.event_type].append(_decayed_weight(item, effective_date))
        need_score = 0.0
        ...
        commercial_score = min(len(event_types & COMMERCIAL_EVENTS) * 4 + (4 if 'factory_or_capacity' in event_types else 0), 10)
        ...
        confidence_score = 10 if len(unique_domains) >= 3 and 'A' in grades else 8 if len(unique_domains) >= 2 and 'B' in grades else 5
        corroboration_bonus = 3 if len(unique_domains) >= 2 and len(event_types) >= 2 else 0
        advertised_penalty = -10 if related_ads else 0
        score_components = [
            ScoreComponent(
                key='need', label='需求信号', points=round(need_score, 1),
                reason=f'{len(seen_events)} 个独立上游事件，按事件类型和时间衰减计分',
                evidence_urls=tuple(dict.fromkeys(item.source_url for item in upstream)),
            ),
            ScoreComponent(
                key='role', label='总监级岗位逻辑', points=round(role_score, 1),
                reason='、'.join(roles),
                evidence_urls=tuple(dict.fromkeys(item.source_url for item in upstream)),
            ),
            ScoreComponent(
                key='timing', label='介入时机', points=round(timing_score, 1),
                reason='尚未发现相关公开广告' if not related_ads else '已发现相关广告，窗口变窄',
                evidence_urls=tuple(item.source_url for item in related_ads),
            ),
            ScoreComponent(
                key='commercial', label='商业化强度', points=round(commercial_score, 1),
                reason=f'商业事件：{", ".join(sorted(event_types & COMMERCIAL_EVENTS)) or "无"}',
                evidence_urls=tuple(dict.fromkeys(
                    item.source_url for item in upstream if item.event_type in COMMERCIAL_EVENTS
                )),
            ),
            ScoreComponent(
                key='confidence', label='证据可信度',
        ...
        if advertised_penalty:
            score_components.append(ScoreComponent(
                key='advertised_penalty', label='公开招聘扣分', points=advertised_penalty,
        ...
        confidence = 'A' if len(unique_domains) >= 3 and 'A' in grades else 'B' if len(unique_domains) >= 2 and 'B' in grades else 'C'
        ...
        if check and check.get('matching_results', 0) == 0:
            ...
            if other_ads:
                ...
                risk_notes.append(f'已发现其他职能的总监级广告：{other_ads_text}；说明企业已进入部分公开招聘阶段。')
        ...
        known_people = list(dict.fromkeys(person for item in items for person in item.people))
        known_organizations = list(dict.fromkeys(org for item in items for org in item.organizations))
        basic_research = {
        ...
        leads.append(CompanyLead(
            ...
            target_roles=roles,
            hiring_thesis=_hiring_thesis(company, direction, event_types, roles),
            ...
            industry_layer=(metadata or {}).get('industry_layers', {}).get(company, 'core'),
            mainland_relevance=(metadata or {}).get('mainland_relevance', {}).get(
                company, '中国大陆招聘市场相关'
            ),
...
def build_late_opportunities(
    ...
    for company, items in grouped.items():
        ...
        director_ads = [
            item
            for item in ads
            if classify_seniority(item.title, item.snippet)[1]
        ]
        if not director_ads:
            continue
        output.append({
...
def _lead_time(upstream: list[Evidence], ads: list[Evidence]) -> int | None:
    if not ads:
        return None
    upstream_dates = [value for item in upstream if (value := _parse_date(item.event_date))]
...
def _hiring_thesis(company: str, direction: str, event_types: set[str], roles: list[str]) -> str:
    drivers: list[str] = []
