...
EVENT_PATTERNS: tuple[tuple[str, str, str], ...] = tuple(
    (signal.name, signal.phase, signal.pattern.pattern) for signal in SIGNALS
)


COMPANY_PATTERNS = (
    re.compile(
        r"(?P<name>[\u4e00-\u9fffA-Za-z0-9·（）()]{2,24}?)(?:完成|获|宣布|计划|启动|签署|拿下|发布|开启)"
    ),
    re.compile(
        r"(?:企业|公司)[“「]?(?P<name>[\u4e00-\u9fffA-Za-z0-9·（）()]{2,24})[”」]?"
    ),
    re.compile(
        r"(?P<name>[A-Z][A-Za-z0-9 .&'’-]{1,50}?)(?:\s+raises?\s+|\s+raised\s+)", re.I
    ),
)
...
def is_plausible_company(name: str, known_seeds: tuple[str, ...] = ()) -> bool:
    ...
    if not 2 <= len(name) <= 16:
        return False
    if name in GENERIC_ENTITY_NAMES or any(
        fragment in name for fragment in INVALID_ENTITY_FRAGMENTS
    ):
        return False
...
def company_local_context(
    company: str, title: str, snippet: str, window: int = 180
) -> str:
...
def grade_source(url: str) -> str:
    ...
    if any(
        ...
        return "A"
    if any(
        host == domain or host.endswith(f".{domain}") for domain in professional_media
    ):
        return "B"
    return "C"
...
def extract_company_candidates(text: str) -> list[str]:
    ...
    for pattern in patterns:
        for match in re.finditer(pattern, text):
            ...
            candidate = re.sub(
                r"^(?:具身智能|人形机器人|国内|全球|企业)", "", candidate
            )
            if (
                candidate
                and is_plausible_company(candidate)
...
class BingRSSCollector:
    ...
    def collect(
        self, direction: str, year: int = 2026, limit_per_query: int = 10
    ) -> list[Evidence]:
        ...
        for query in discovery_queries:
            for result in self.search(query, limit=limit_per_query):
                ...
                for candidate in extract_company_candidates(combined):
                    if (
                        is_plausible_company(candidate, profile.seed_companies)
                        and candidate not in candidates
                    ):
                        candidates.append(candidate)
            ...
            for query in queries:
                for result in self.search(query, limit=limit_per_query):
                    ...
                    if not result.url or key in seen:
                        continue
                    local_context = result_relevant_to_company(
                        company, result.title, result.snippet
                    )
                    if not local_context:
                        continue
                    ...
                    evidence.append(
                        Evidence(
                            ...
                            phase=phase,
                            event_date=_event_date_from_text(
                                local_context, result.published_at
                            ),
                            title=result.title,
    ...
    def collect_routes(
        self, company: str, direction: str, limit_per_query: int = 8
    ) -> list[OutreachRoute]:
        queries = (
        ...
        for page in body.get("webpages", [])[:limit]:
            results.append(
                SearchResult(
                    ...
                    snippet=_clean_html(
                        str(page.get("snippet") or page.get("summary") or "")
                    ),
    ...
    def search(self, query: str, limit: int = 10) -> list[SearchResult]:
        ...
        return [
            SearchResult(
                ...
                published_at=str(
                    item.get("publishedDate") or item.get("pubdate") or ""
                ),
            )
...
def load_replay(path: str | Path, direction: str) -> tuple[list[Evidence], dict]:
    ...
    for lead in payload:
        ...
        for item in lead.get("evidence", []):
            ...
            seen.add(key)
            event_date = _event_date_from_text(
                local_context, str(item.get("event_date") or "")
            )
        ...
        for item in lead.get("outreach_routes", []):
            ...
            if normalized is None:
                continue
            if normalized["kind"] == "investor" and not investor_supported(
                normalized["target"], evidence_corpus
            ):
                continue
...
def normalize_replayed_route(route: dict) -> dict | None:
    ...
    if kind == "investor":
        ...
        if not 2 <= len(target) <= 70:
            return None
        normalized["path"] = (
            f"请 Michael Page 内部或共同 LP/被投企业联系人请求 {target} 引荐公司创始团队。"
        )
        ...
        if not 2 <= len(organization) <= 40 or any(
            value in organization for value in invalid_organizations
        ):
            return None
        target = f"{person}—{organization}"
        if kind == "alumni_or_academic":
            normalized["path"] = (
                f"优先查找 Michael Page、客户或候选人网络中的 {organization} 校友/师门关系，请求暖介绍 {person}。"
            )
        else:
            normalized["path"] = (
                f"在经许可的 Michael Page 关系网络中查找 {organization} 前同事，请求对 {person} 的暖介绍。"
            )
    else:
        return None
    invalid = (
...
def investor_supported(target: str, evidence_text: str) -> bool:
    ...
    return any(
        component.strip() in evidence_text
...
def collect_josint(
    ...
    *,
    ...
    effective_topics = tuple(topics or (direction,))
    terms = tuple(
        dict.fromkeys(
    ...
    for row in rows:
        ...
        if not any(term.lower() in text.lower() for term in terms):
            continue
        _, eligible, _ = classify_seniority(
            str(row["title"] or ""), str(row["jd_text"] or "")
        )
        if not eligible:
            continue
...
def _company_hint(description: str) -> str | None:
    match = re.search(
        r"([\u4e00-\u9fffA-Za-z0-9·（）()]{2,30}(?:公司|集团|机器人|科技))", description
    )
    return match.group(1) if match else None


def infer_routes_from_text(
    text: str, source_url: str, company: str = ""
) -> list[OutreachRoute]:
    compact = re.sub(r"\s+", " ", text)
    routes: list[OutreachRoute] = []
    investment = re.search(
        r"(?:由|获)([^，,。；]{2,70}?)(?:领投|联合投资|投资)", compact
    )
    equity_target = None
